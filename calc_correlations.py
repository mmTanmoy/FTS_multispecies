...
ks1d = 2.*np.pi*np.fft.fftfreq( Nx, dx )
kx, ky, kz = np.meshgrid( ks1d, ks1d, ks1d)
Gamma = np.exp( -(kx**2 + ky**2 + kz**2) * a**2 / 2 )
...
def cal_correlations(w, rho):
    ...
    rho = np.array([ ift( Gamma * ft(rho[i]) ) for i in range(rho.shape[0]) ])
...
seqs          = 'sv28_sv24_'  # The sequence-pair string
runs          = 40            # Number of independent runs
starting_step = 20000         # Initial CL-step of sampling
ending_step   = 50001         # Ending CL-step of sampling
interval      = 500           # Sampling interval
